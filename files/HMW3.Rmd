...
```{r, include=TRUE, echo=FALSE}
liq2017[Month==1 | Month==12, out:=0]
summary(lm(Firms ~ as.factor(Month) + out, liq2017))
```
...
```{r, include=TRUE, echo=FALSE}
checkresiduals(lm(Firms ~ as.factor(Month) + out, liq2017))
```
...
summary(lm(Firms ~ as.factor(Month) + out + Loan, liq2017))
```
...
```{r, include=TRUE, echo=FALSE}
checkresiduals(lm(Firms ~ as.factor(Month) + out + Loan, liq2017))
```
...
summary(lm(Firms ~ as.factor(Month) + out + Loan + RUS2, liq2017))
```
...
```{r, include=TRUE, echo=FALSE}
summary(lm(Firms ~ as.factor(Month) + as.factor(Year) + out + Loan, liq2017))
```
...
```{r, include=TRUE, echo=FALSE}
summary(lm(Firms ~ as.factor(Month) + as.factor(Year) + out + RUS2, liq2017))
```
...
```{r, include=TRUE, echo=FALSE}
checkresiduals(lm(Firms ~ as.factor(Month) + as.factor(Year) + out + RUS2, liq2017))
```

By the findings, there seems to be no serial correlation that is statistically significant. Moreover, the residuals can be easily fitted to a normal distribution. The volatility of residuals in Year 2020 is reduced. Nonetheless, in the last few months of 2017 the errors have become volatile. As a good model with minimal error and acceptable residual statistics has been found. The discussion will be continued by commenting on findings that will be obtained by applying the model on the dataset.
...
```{r, include=TRUE, echo=FALSE}
model = lm(Firms ~ as.factor(Month) + as.factor(Year) + out + RUS2, liq2017)
